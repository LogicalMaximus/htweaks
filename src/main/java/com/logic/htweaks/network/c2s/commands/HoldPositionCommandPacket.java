...
public class HoldPositionCommandPacket implements IMessage {
    public HoldPositionCommandPacket() {

    }

    public HoldPositionCommandPacket(FriendlyByteBuf friendlyByteBuf) {
        ...
        if(context != null) {
            ...
            for(AbstractScavEntity scav : entities) {
                if(((IPatrolLeader)scav).shouldFollowCommands(player) && scav.getVehicle() != null) {
                    boolean isHoldingPosition = Boolean.TRUE.equals(BrainUtils.getMemory(scav, ModMemoryTypes.SHOULD_HOLD_POSITION.get()));

                    BrainUtils.setMemory(scav.getBrain(), ModMemoryTypes.SHOULD_HOLD_POSITION.get(), !isHoldingPosition);
                }
            }
        }
    }
